...
export default AccessDevice = () => {
  ...
  const handleDevicePress = (deviceId) => {
    if (deviceId === 2) { // ID của thiết bị "Light"
      navigation.navigate("infoDenTran_Home2"); // Chuyển hướng đến màn hình infoDenTran.js
    }
    if (deviceId === 4) { // ID của thiết bị "Quat"
      navigation.navigate("infoQuat_Home2"); // Chuyển hướng đến màn hình infoQuat.js
    }
  };
